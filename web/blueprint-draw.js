// The blueprint's layout engine and drawing, shared.
//
// This was inside blueprint.js and moved out the moment a second page wanted to
// draw the same datapath: halfshot.js shows the same units, rails and switches
// with a frame's values on them. Same reasoning as sch-draw.js and
// block-palette.js, and the same stake: two pages laying the accumulator out
// from two copies of this code would eventually put it in two different places,
// and a reader comparing them would have no way to tell which was lying.
//
// Nothing here decides what the 6502 contains. The units, the edges, the control
// line on each edge and the order things sit in all arrive in blueprint.json,
// derived from switch topology by crates/v6502-netlist/src/blueprint.rs. This
// file is a layout engine and a set of symbols, and it should stay that way: if
// a fact about the chip ever gets hardcoded below, it is in the wrong file.
//
// It knows nothing about pages, machines or state. `layOut` takes the parsed
// JSON and returns positions; `draw` puts them into an SVG; `unitValue` reads a
// unit's byte out of a node-level array; whoever owns the SVG paints values and
// open switches onto it.
...
export const el = (name, attrs = {}, text) => {
...
export const shortControl = (name) => {
...
export const label = (u) => LABELS[u.name] || u.name.toUpperCase();
export const longLabel = (u) => LONG[u.name] || u.name;

export function draw(svg, bp, layout) {
...
export function placeLabels(svg) {
  const hits = (a, b) =>
...
export const countBits = (m) => m.toString(2).split('').filter((c) => c === '1').length;
...
export function unitValue(levels, unit) {
  let v = 0;
  let known = 0;
  for (let b = 0; b < 8; b++) {
