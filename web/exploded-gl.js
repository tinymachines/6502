...
import { BLOCK_COLOR } from './block-palette.js';
...
// The block palette is shared with the schematic, which draws the same blocks
// in SVG and cannot import a WebGL renderer to find out what colour they are.
// Re-exported here so `exploded.js` keeps importing it from one place.
// Re-exported so `exploded.js` keeps importing it from one place. The path is
// named once in this file on purpose: build-web.py rewrites it with
// replace_once, whose whole value is failing when there is more than one.
export { BLOCK_COLOR };
