...
// The status register and the flag logic (flag-logic.js): each flag bit with
// the cone that sets, clears, loads and holds it, the enables the flags share,
// and the Pout bits that push P onto the data bus. Beads, one hue per flag;
// a flag's card reads the bit off the chip's P and names what it reads.
export const FLAGS_R = 90;
// The address latches (address-latches.js): the two latches that hold the
// address on the pins, seven nodes a bit from the bus's inverter to the pad
// driver; the two load lines with their cones; and the constant generators
// that force address bits (the vector bits low, zero page and stack high).
export const ALAT_R = 90;
// The ALU and its adder (alu-slices.js): eight bit slices by the die's own
// per-bit names, the shared generate terms, the two inputs, the carry in,
...
// The instruction register and predecode (ir-predecode.js): the predecode
// latch, the load path under fetch, the register, the predecoder, irline3,
// clearIR and fetch. Beads in the instruction register's own hues.
export const IRP_R = 90;
// The special bus (special-bus.js): the eight bits, the decimal-adjusted
// copies, and the thirteen lines by the direction read off the far side of
// their switches: onto the bus, off it, or joining it to the data bus.
export const SBUS_R = 90;
// The store-data pipeline (store-pipeline.js): the detect over the data-cycle
// and RMW terms, and the two latches SD1 and SD2 the simulator's own timing
...
// The last three (pc-pipe-sync.js): the program counter's own storage with
// its six lines, the 52-latch pipeline file, and the SYNC generator.
export const PCR_R = 90;
const PCR_COLOR = { pcl: 'rgb(140 190 255)', pch: 'rgb(120 170 235)', pclp: 'rgb(170 215 255)', pchp: 'rgb(150 195 245)', PCLPCL: 'rgb(255 190 90)', ADLPCL: 'rgb(150 235 140)', PCLADL: 'rgb(200 150 255)', PCHPCH: 'rgb(255 170 80)', ADHPCH: 'rgb(130 225 160)', PCHADH: 'rgb(220 170 255)' };
export const PIPE_R = 80;
const PIPE_COLOR = { named: 'rgb(255 210 120)', unk: 'rgb(170 175 190)' };
export const SYNC_R = 150;
const SYNC_COLOR = 'rgb(120 225 255)';
...
const SBUS_LABEL = { sb: 'SB', dasb: 'adjusted SB', onto: 'onto SB', off: 'off SB', link: 'SB-DB link', SBADH: 'SBADH' };
const IRP_COLOR = { pd: 'rgb(120 225 200)', load: 'rgb(240 240 255)', ir: 'rgb(255 200 110)', pre: 'rgb(200 150 255)', irline3: 'rgb(130 190 255)', clear: 'rgb(255 130 120)', fetch: 'rgb(255 180 90)' };
const IRP_LABEL = { pd: 'predecode', load: 'IR load', ir: 'IR', pre: 'predecoder', irline3: 'irline3', clear: 'clearIR', fetch: 'fetch' };
...
const ALAT_COLOR = { abl: 'rgb(120 170 255)', abh: 'rgb(150 210 255)', 'ADL/ABL': 'rgb(255 190 90)', 'ADH/ABH': 'rgb(255 210 130)', low: 'rgb(200 150 255)', high: 'rgb(220 170 255)' };
const FLAGS_COLOR = { C: 'rgb(255 200 90)', Z: 'rgb(170 230 170)', I: 'rgb(255 130 120)', D: 'rgb(255 160 80)', B: 'rgb(180 185 200)', V: 'rgb(200 150 255)', N: 'rgb(130 190 255)', shared: 'rgb(240 240 255)', out: 'rgb(120 225 255)' };
const FLAGS_WHAT = { C: 'carry: from the ALU\'s carry out, SEC and CLC, the shifts, or the data bus on PLP and RTI', Z: 'zero: DBZ, a NOR over all eight data-bus bits, or the data bus on PLP and RTI', I: 'interrupt disable: SEI, CLI, the BRK sequence, or the data bus', D: 'decimal: SED, CLD, or the data bus', B: 'break: not a stored bit at all, an inverter of D1x1, the timing chain\'s BRK-against-interrupt distinction', V: 'overflow: the ALU\'s overflow, the S.O. pin, CLV, BIT, or the data bus', N: 'negative: DBNeg, the data bus\'s bit 7' };
...
const state = {
  ...
  incrRegions: true,
  flagGroups: null,  // [{id, nodes, ...}] each flag's logic, the shared enables, Pout
  flagRegionData: null,
  flag: null,        // the selected flag group id, or null
  flagRegions: true,
  alats: null,       // [{id, kind, nodes, ...}] the address latches, their lines, the constants
  alatRegionData: null,
  alat: null,        // the selected address-latch group id, or null
  alatRegions: true,
  alus: null,        // [{id, kind, nodes, ...}] the ALU's slices, inputs, ends and line groups
  aluRegionData: null,
  ...
  dbusRegions: true,
  irps: null,        // [{id, kind, nodes, ...}] the predecode, the load path, the register, the predecoder, the named extras
  irpRegionData: null,
  irp: null,         // the selected IR group id, or null
  irpRegions: true,
  sbuses: null,      // [{id, kind, nodes, ...}] the special bus, the adjusted copies, the lines by direction
  sbusRegionData: null,
  sbus: null,        // the selected special-bus group id, or null
  sbusRegions: true,
  sdps: null,        // [{id, node, nodes, ...}] the store-data pipeline
  sdpRegionData: null,
  ...
  rdyRegions: true,
  pcrs: null,        // [{id, kind, nodes, ...}] the PC's storage and its lines
  pcrRegionData: null,
  pcr: null,
  pcrRegions: true,
  pipeGroups: null,  // [{id, nodes, ...}] the pipeline latch file
  pipeRegionData: null,
  pipe: null,
  pipeRegions: true,
  syncs: null,       // [{id, nodes, ...}] the SYNC generator
  syncRegionData: null,
  sync: null,
  syncRegions: true,
  pinRegions: true,
...
/** A generic small container kind: list, regions, draw, hit test. */
function makeKind(cfg) {
  // cfg: { key, cls, radius, colorOf, labelOf, stateList, stateData }
  const list = () => {
    if (state[cfg.stateList]) return state[cfg.stateList];
    const keep = (ns) => ns.filter((n) => state.pos.has(n));
    const out = cfg.build().map((g) => ({ ...g, nodes: keep(g.nodes), label: cfg.labelOf(g) }));
    state[cfg.stateList] = out;
    return out;
  };
  const data = () => {
    if (state[cfg.stateData]) return state[cfg.stateData];
    const l = list();
    const idx = new Array(2048).fill(-1);
    l.forEach((c, i) => { for (const n of c.nodes) idx[n] = i; });
    const d = blockRegions(state.pos, idx, l.map((_, i) => i), state.bounds, { radius: cfg.radius, cell: 25 });
    const out = new Map();
    l.forEach((c, i) => out.set(c.id, d.get(i)));
    state[cfg.stateData] = out;
    return out;
  };
  const draw = (g) => {
    const d = data();
    for (const c of list()) {
      const r = d.get(c.id);
      const css = cfg.colorOf(c);
      const path = el('path', { d: loopsToPath(r.loops), class: `${cfg.cls}` + (state[cfg.key] === c.id ? ' sel' : ''), 'fill-rule': 'evenodd',
                                [`data-${cfg.key}`]: c.id, style: `--bc: ${css}` }, g);
      path.setAttribute('aria-label', `${c.label}, ${c.nodes.length} nodes`);
      if (r.label) {
        const t = el('text', { x: r.label.x, y: r.label.y - cfg.radius - 24, class: `${cfg.cls}-lb` + (state[cfg.key] === c.id ? ' sel' : ''), [`data-${cfg.key}`]: c.id, style: `--bc: ${css}` }, g);
        t.textContent = c.label;
      }
    }
    g.classList.toggle('off', !state[`${cfg.key}Regions`]);
  };
  const at = (pt) => {
    const d = data();
    let best = null, bd = Infinity;
    for (const c of list()) {
      if (!inRegion(pt, d.get(c.id).loops)) continue;
      for (const n of c.nodes) {
  ...
  return { list, data, draw, at };
}

const PCR = makeKind({ key: 'pcr', cls: 'tc-pg2', radius: PCR_R, stateList: 'pcrs', stateData: 'pcrRegionData',
  colorOf: (c) => PCR_COLOR[c.id] || STAGE_COLOR.any,
  labelOf: (g) => g.kind === 'byte' ? `PC ${g.id}` : g.id,
  build: () => pcRegister(state.sch).groups });
const PIPES = makeKind({ key: 'pipe', cls: 'tc-pp', radius: PIPE_R, stateList: 'pipeGroups', stateData: 'pipeRegionData',
  colorOf: (c) => PIPE_COLOR[c.id] || STAGE_COLOR.any,
  labelOf: (g) => g.id === 'unk' ? 'pipeUNK file' : 'pipe latches',
  build: () => pipeFile(state.sch).groups });
const SYNCG = makeKind({ key: 'sync', cls: 'tc-sy', radius: SYNC_R, stateList: 'syncs', stateData: 'syncRegionData',
  colorOf: () => SYNC_COLOR, labelOf: () => 'SYNC generator',
  build: () => syncGen(state.sch).groups });

/** The ready logic, from ready-logic.js. *//** The ready logic, from ready-logic.js. */
function rdyList() {
...
/** The store-data pipeline, from store-pipeline.js. */
function sdpList() {
  if (state.sdps) return state.sdps;
  const { pos, sch } = state;
  const r = storePipeline(sch);
  const keep = (ns) => ns.filter((n) => pos.has(n));
  const out = r.groups.map((g) => ({ ...g, nodes: keep(g.nodes), label: SDP_LABEL[g.id] || g.id }));
  state.sdps = out;
  state.sdpStats = { groups: out.map((g) => [g.id, g.nodes.length]) };
  return out;
}

function sdpRegionData() {
  if (state.sdpRegionData) return state.sdpRegionData;
  const list = sdpList();
  const idx = new Array(2048).fill(-1);
  list.forEach((c, i) => { for (const n of c.nodes) idx[n] = i; });
  const data = blockRegions(state.pos, idx, list.map((_, i) => i), state.bounds, { radius: SDP_R, cell: 25 });
  const out = new Map();
  list.forEach((c, i) => out.set(c.id, data.get(i)));
  state.sdpRegionData = out;
  return out;
}

function drawSdpRegions(g) {
  const data = sdpRegionData();
  for (const c of sdpList()) {
    const r = data.get(c.id);
    const css = SDP_COLOR[c.id] || STAGE_COLOR.any;
    const path = el('path', { d: loopsToPath(r.loops), class: 'tc-wg' + (state.sdp === c.id ? ' sel' : ''), 'fill-rule': 'evenodd',
                              'data-sdp': c.id, style: `--bc: ${css}` }, g);
    path.setAttribute('aria-label', `store pipeline, ${c.label}, ${c.nodes.length} nodes`);
    if (r.label) {
      const t = el('text', { x: r.label.x, y: r.label.y - SDP_R - 24, class: 'tc-wg-lb' + (state.sdp === c.id ? ' sel' : ''), 'data-sdp': c.id, style: `--bc: ${css}` }, g);
      t.textContent = c.label;
    }
  }
  g.classList.toggle('off', !state.sdpRegions);
}
...
/** The special bus, from special-bus.js. */
function sbusList() {
  if (state.sbuses) return state.sbuses;
  const { pos, sch } = state;
  const r = specialBus(sch);
  const keep = (ns) => ns.filter((n) => pos.has(n));
  const out = r.groups.map((g) => ({ ...g, nodes: keep(g.nodes), label: SBUS_LABEL[g.id] || g.id }));
  state.sbuses = out;
  state.sbusStats = { groups: out.map((g) => [g.id, g.nodes.length]), precharge: r.precharge, lines: (out.find((g) => g.id === 'sb') || { lines: [] }).lines.length };
  return out;
}

function sbusRegionData() {
  if (state.sbusRegionData) return state.sbusRegionData;
  const list = sbusList();
  const idx = new Array(2048).fill(-1);
  list.forEach((c, i) => { for (const n of c.nodes) idx[n] = i; });
  const data = blockRegions(state.pos, idx, list.map((_, i) => i), state.bounds, { radius: SBUS_R, cell: 25 });
  const out = new Map();
  list.forEach((c, i) => out.set(c.id, data.get(i)));
  state.sbusRegionData = out;
  return out;
}

function drawSbusRegions(g) {
  const data = sbusRegionData();
  for (const c of sbusList()) {
    const r = data.get(c.id);
    const css = SBUS_COLOR[c.id] || STAGE_COLOR.any;
    const path = el('path', { d: loopsToPath(r.loops), class: 'tc-yg' + (state.sbus === c.id ? ' sel' : ''), 'fill-rule': 'evenodd',
                              'data-sbus': c.id, style: `--bc: ${css}` }, g);
    path.setAttribute('aria-label', `special bus, ${c.label}, ${c.nodes.length} nodes`);
    if (r.label) {
      const t = el('text', { x: r.label.x, y: r.label.y - SBUS_R - 24, class: 'tc-yg-lb' + (state.sbus === c.id ? ' sel' : ''), 'data-sbus': c.id, style: `--bc: ${css}` }, g);
      t.textContent = c.label;
    }
  }
  g.classList.toggle('off', !state.sbusRegions);
}
...
/** The instruction register and predecode, from ir-predecode.js. */
function irpList() {
  if (state.irps) return state.irps;
  const { pos, sch } = state;
  const r = irPredecode(sch);
  const keep = (ns) => ns.filter((n) => pos.has(n));
  const out = r.groups.map((g) => ({ ...g, nodes: keep(g.nodes), label: IRP_LABEL[g.id] || g.id }));
  state.irps = out;
  state.irpStats = { groups: out.map((g) => [g.id, g.nodes.length]), terms: (out.find((g) => g.id === 'ir') || { feeds: [] }).feeds.filter((n) => sch.nodeRole[n] === 1).length, line3: (out.find((g) => g.id === 'irline3') || { feeds: [] }).feeds.filter((n) => sch.nodeRole[n] === 1).length };
  return out;
}

function irpRegionData() {
  if (state.irpRegionData) return state.irpRegionData;
  const list = irpList();
  const idx = new Array(2048).fill(-1);
  list.forEach((c, i) => { for (const n of c.nodes) idx[n] = i; });
  const data = blockRegions(state.pos, idx, list.map((_, i) => i), state.bounds, { radius: IRP_R, cell: 25 });
  const out = new Map();
  list.forEach((c, i) => out.set(c.id, data.get(i)));
  state.irpRegionData = out;
  return out;
}

function drawIrpRegions(g) {
  const data = irpRegionData();
  for (const c of irpList()) {
    const r = data.get(c.id);
    const css = IRP_COLOR[c.id] || STAGE_COLOR.any;
    const path = el('path', { d: loopsToPath(r.loops), class: 'tc-ig2' + (state.irp === c.id ? ' sel' : ''), 'fill-rule': 'evenodd',
                              'data-irp': c.id, style: `--bc: ${css}` }, g);
    path.setAttribute('aria-label', `instruction register, ${c.label}, ${c.nodes.length} nodes`);
    if (r.label) {
      const t = el('text', { x: r.label.x, y: r.label.y - IRP_R - 24, class: 'tc-ig2-lb' + (state.irp === c.id ? ' sel' : ''), 'data-irp': c.id, style: `--bc: ${css}` }, g);
      t.textContent = c.label;
    }
  }
  g.classList.toggle('off', !state.irpRegions);
}
...
/** The ALU, from alu-slices.js: eight slices, the shared terms, inputs, ends, line groups. */
function aluList() {
  if (state.alus) return state.alus;
  const { pos, sch } = state;
  const r = aluSlices(sch);
  const keep = (ns) => ns.filter((n) => pos.has(n));
  const out = r.groups.map((g) => ({ ...g, nodes: keep(g.nodes), label: g.kind === 'bit' ? `alu ${g.bit}` : ALU_LABEL[g.id] || g.id }));
  state.alus = out;
  state.aluStats = { groups: out.map((g) => [g.id, g.nodes.length]), clocks: r.clocks.map((n) => sch.names[n] || `#${n}`) };
  return out;
}

function aluRegionData() {
  if (state.aluRegionData) return state.aluRegionData;
  const list = aluList();
  const idx = new Array(2048).fill(-1);
  list.forEach((c, i) => { for (const n of c.nodes) idx[n] = i; });
  const data = blockRegions(state.pos, idx, list.map((_, i) => i), state.bounds, { radius: ALU_R, cell: 25 });
  const out = new Map();
  list.forEach((c, i) => out.set(c.id, data.get(i)));
  state.aluRegionData = out;
  return out;
}

function drawAluRegions(g) {
  const data = aluRegionData();
  for (const c of aluList()) {
    const r = data.get(c.id);
    const css = ALU_COLOR[c.id] || STAGE_COLOR.any;
    const path = el('path', { d: loopsToPath(r.loops), class: 'tc-ug' + (state.alu === c.id ? ' sel' : ''), 'fill-rule': 'evenodd',
                              'data-alu': c.id, style: `--bc: ${css}` }, g);
    path.setAttribute('aria-label', `ALU, ${c.label}, ${c.nodes.length} nodes`);
    if (r.label) {
      const t = el('text', { x: r.label.x, y: r.label.y - ALU_R - 24, class: 'tc-ug-lb' + (state.alu === c.id ? ' sel' : ''), 'data-alu': c.id, style: `--bc: ${css}` }, g);
      t.textContent = c.label;
    }
  }
  g.classList.toggle('off', !state.aluRegions);
}
...
/** The address latches, from address-latches.js: two halves, two load lines, two constant groups. */
function alatList() {
  if (state.alats) return state.alats;
  const { pos, sch } = state;
  const r = addressLatches(sch);
  const keep = (ns) => ns.filter((n) => pos.has(n));
  const out = [];
  for (const h of r.halves) out.push({ id: h.id, kind: 'half', nodes: keep(h.nodes), reads: h.reads, feeds: h.feeds, label: h.id.toUpperCase() });
  for (const L of r.lines) out.push({ id: L.id, kind: 'line', half: L.half, node: L.node, nodes: keep(L.nodes), reads: L.reads, switches: L.switches, onHalf: L.onHalf, label: L.id });
  for (const c of r.consts) out.push({ id: c.id, kind: 'const', roots: c.roots, nodes: keep(c.nodes), reads: c.reads, label: c.id === 'low' ? 'ADL const' : 'ADH const' });
  state.alats = out;
  state.alatStats = { halves: r.halves.map((h) => [h.id, h.nodes.length]), lines: r.lines.map((L) => [L.id, L.nodes.length, L.onHalf]), consts: r.consts.map((c) => [c.id, c.nodes.length, c.roots.length]), depth: r.depth };
  return out;
}

function alatRegionData() {
  if (state.alatRegionData) return state.alatRegionData;
  const list = alatList();
  const idx = new Array(2048).fill(-1);
  list.forEach((c, i) => { for (const n of c.nodes) idx[n] = i; });
  const data = blockRegions(state.pos, idx, list.map((_, i) => i), state.bounds, { radius: ALAT_R, cell: 25 });
  const out = new Map();
  list.forEach((c, i) => out.set(c.id, data.get(i)));
  state.alatRegionData = out;
  return out;
}

function drawAlatRegions(g) {
  const data = alatRegionData();
  for (const c of alatList()) {
    const r = data.get(c.id);
    const css = ALAT_COLOR[c.id] || STAGE_COLOR.any;
    const path = el('path', { d: loopsToPath(r.loops), class: 'tc-ag' + (state.alat === c.id ? ' sel' : ''), 'fill-rule': 'evenodd',
                              'data-alat': c.id, style: `--bc: ${css}` }, g);
    path.setAttribute('aria-label', `address latches, ${c.label}, ${c.nodes.length} nodes`);
    if (r.label) {
      const t = el('text', { x: r.label.x, y: r.label.y - ALAT_R - 24, class: 'tc-ag-lb' + (state.alat === c.id ? ' sel' : ''), 'data-alat': c.id, style: `--bc: ${css}` }, g);
      t.textContent = c.label;
    }
  }
  g.classList.toggle('off', !state.alatRegions);
}
...
/** The status register and flag logic, from flag-logic.js. */
function flagsList() {
  if (state.flagGroups) return state.flagGroups;
  const { pos, sch } = state;
  const r = flagLogic(sch);
  const keep = (ns) => ns.filter((n) => pos.has(n));
  const out = r.groups.map((g) => ({ ...g, nodes: keep(g.nodes) }));
  state.flagGroups = out;
  state.flagStats = { groups: out.map((g) => [g.id, g.nodes.length]), depth: r.depth, clocks: r.clocks.map((n) => sch.names[n] || `#${n}`) };
  return out;
}

function flagsRegionData() {
  if (state.flagRegionData) return state.flagRegionData;
  const list = flagsList();
  const idx = new Array(2048).fill(-1);
  list.forEach((c, i) => { for (const n of c.nodes) idx[n] = i; });
  const data = blockRegions(state.pos, idx, list.map((_, i) => i), state.bounds, { radius: FLAGS_R, cell: 25 });
  const out = new Map();
  list.forEach((c, i) => out.set(c.id, data.get(i)));
  state.flagRegionData = out;
  return out;
}

function drawFlagsRegions(g) {
  const data = flagsRegionData();
  for (const c of flagsList()) {
    const r = data.get(c.id);
    const css = FLAGS_COLOR[c.id] || STAGE_COLOR.any;
    const path = el('path', { d: loopsToPath(r.loops), class: 'tc-fg' + (state.flag === c.id ? ' sel' : ''), 'fill-rule': 'evenodd',
                              'data-flag': c.id, style: `--bc: ${css}` }, g);
    path.setAttribute('aria-label', `flag logic, ${c.id}, ${c.nodes.length} nodes`);
    if (r.label) {
      const t = el('text', { x: r.label.x, y: r.label.y - FLAGS_R - 24, class: 'tc-fg-lb' + (state.flag === c.id ? ' sel' : ''), 'data-flag': c.id, style: `--bc: ${css}` }, g);
      t.textContent = c.kind === 'flag' ? `flag ${c.id}` : c.id === 'out' ? 'P out' : 'flags shared';
    }
  }
  g.classList.toggle('off', !state.flagRegions);
}
...
function selectBlock(b, { fly = false } = {}) {
  state.block = b;
  if (b !== null) { state.stem = null; state.cluster = null; state.stage = null; state.control = null; state.pin = null; state.chain = null; state.clock = null; state.intr = null; state.branch = null; state.decimal = null; state.reg = null; state.incr = null; state.flag = null; state.alat = null; state.alu = null; state.dbus = null; state.irp = null; state.sbus = null; state.sdp = null; state.rdy = null; state.pcr = null; state.pipe = null; state.sync = null; }
  const { sch } = state;
...
function selectStem(stem, { fly = false } = {}) {
  ...
  if (s) { state.block = null; state.cluster = null; state.stage = null; state.control = null; state.pin = null; state.chain = null; state.clock = null; state.intr = null; state.branch = null; state.decimal = null; state.reg = null; state.incr = null; state.flag = null; state.alat = null; state.alu = null; state.dbus = null; state.irp = null; state.sbus = null; state.sdp = null; state.rdy = null; state.pcr = null; state.pipe = null; state.sync = null; }
...
function selectCluster(id, { fly = false } = {}) {
  ...
  if (c) { state.block = null; state.stem = null; state.stage = null; state.control = null; state.pin = null; state.chain = null; state.clock = null; state.intr = null; state.branch = null; state.decimal = null; state.reg = null; state.incr = null; state.flag = null; state.alat = null; state.alu = null; state.dbus = null; state.irp = null; state.sbus = null; state.sdp = null; state.rdy = null; state.pcr = null; state.pipe = null; state.sync = null; }
...
function selectStage(id, { fly = false } = {}) {
  ...
  if (c) { state.block = null; state.stem = null; state.cluster = null; state.control = null; state.pin = null; state.chain = null; state.clock = null; state.intr = null; state.branch = null; state.decimal = null; state.reg = null; state.incr = null; state.flag = null; state.alat = null; state.alu = null; state.dbus = null; state.irp = null; state.sbus = null; state.sdp = null; state.rdy = null; state.pcr = null; state.pipe = null; state.sync = null; }
  const members = c ? new Set(c.nodes) : null;
  applySelection(members ? (n) => members.has(n) : null, fly);
}
...
function selectRegs(id, { fly = false } = {}) {
  ...
  if (c) { state.block = null; state.stem = null; state.cluster = null; state.stage = null; state.control = null; state.pin = null; state.chain = null; state.clock = null; state.intr = null; state.branch = null; state.decimal = null; state.incr = null; state.flag = null; state.alat = null; state.alu = null; state.dbus = null; state.irp = null; state.sbus = null; state.sdp = null; state.rdy = null; state.pcr = null; state.pipe = null; state.sync = null; }
  const members = c ? new Set(c.nodes) : null;
  applySelection(members ? (n) => members.has(n) : null, fly);
}

/** One selector for the three small kinds: clears everything, sets its own. */
function selectSmall(key, listFn, id, { fly = false } = {}) {
  const c = id === null ? null : listFn().find((x) => x.id === id);
  for (const k of ['block', 'stem', 'cluster', 'stage', 'control', 'pin', 'chain', 'clock', 'intr', 'branch', 'decimal', 'reg', 'incr', 'flag', 'alat', 'alu', 'dbus', 'irp', 'sbus', 'sdp', 'rdy', 'pcr', 'pipe', 'sync']) state[k] = null;
  state[key] = c ? c.id : null;
  const members = c ? new Set(c.nodes) : null;
  applySelection(members ? (n) => members.has(n) : null, fly);
}
const selectPcr = (id, o) => selectSmall('pcr', PCR.list, id, o);
const selectPipe = (id, o) => selectSmall('pipe', PIPES.list, id, o);
const selectSync = (id, o) => selectSmall('sync', SYNCG.list, id, o);
...
function selectRdy(id, { fly = false } = {}) {
  ...
  if (c) { state.block = null; state.stem = null; state.cluster = null; state.stage = null; state.control = null; state.pin = null; state.chain = null; state.clock = null; state.intr = null; state.branch = null; state.decimal = null; state.reg = null; state.incr = null; state.flag = null; state.alat = null; state.alu = null; state.dbus = null; state.irp = null; state.sbus = null; state.sdp = null; state.pcr = null; state.pipe = null; state.sync = null; }
  const members = c ? new Set(c.nodes) : null;
  applySelection(members ? (n) => members.has(n) : null, fly);
}

/** Select one group of the store-data pipeline, as a set. */
function selectSdp(id, { fly = false } = {}) {
  const c = id === null ? null : sdpList().find((x) => x.id === id);
  state.sdp = c ? c.id : null;
  if (c) { state.block = null; state.stem = null; state.cluster = null; state.stage = null; state.control = null; state.pin = null; state.chain = null; state.clock = null; state.intr = null; state.branch = null; state.decimal = null; state.reg = null; state.incr = null; state.flag = null; state.alat = null; state.alu = null; state.dbus = null; state.irp = null; state.sbus = null; state.rdy = null; state.pcr = null; state.pipe = null; state.sync = null; }
  const members = c ? new Set(c.nodes) : null;
  applySelection(members ? (n) => members.has(n) : null, fly);
}

/** Select one group of the special bus, as a set. */
function selectSbus(id, { fly = false } = {}) {
  const c = id === null ? null : sbusList().find((x) => x.id === id);
  state.sbus = c ? c.id : null;
  if (c) { state.block = null; state.stem = null; state.cluster = null; state.stage = null; state.control = null; state.pin = null; state.chain = null; state.clock = null; state.intr = null; state.branch = null; state.decimal = null; state.reg = null; state.incr = null; state.flag = null; state.alat = null; state.alu = null; state.dbus = null; state.irp = null; state.sdp = null; state.rdy = null; state.pcr = null; state.pipe = null; state.sync = null; }
  const members = c ? new Set(c.nodes) : null;
  applySelection(members ? (n) => members.has(n) : null, fly);
}

/** Select one group of the instruction register and predecode, as a set. */
function selectIrp(id, { fly = false } = {}) {
  const c = id === null ? null : irpList().find((x) => x.id === id);
  state.irp = c ? c.id : null;
  if (c) { state.block = null; state.stem = null; state.cluster = null; state.stage = null; state.control = null; state.pin = null; state.chain = null; state.clock = null; state.intr = null; state.branch = null; state.decimal = null; state.reg = null; state.incr = null; state.flag = null; state.alat = null; state.alu = null; state.dbus = null; state.sbus = null; state.sdp = null; state.rdy = null; state.pcr = null; state.pipe = null; state.sync = null; }
  const members = c ? new Set(c.nodes) : null;
  applySelection(members ? (n) => members.has(n) : null, fly);
}
...
function selectDbus(id, { fly = false } = {}) {
  ...
  if (c) { state.block = null; state.stem = null; state.cluster = null; state.stage = null; state.control = null; state.pin = null; state.chain = null; state.clock = null; state.intr = null; state.branch = null; state.decimal = null; state.reg = null; state.incr = null; state.flag = null; state.alat = null; state.alu = null; state.irp = null; state.sbus = null; state.sdp = null; state.rdy = null; state.pcr = null; state.pipe = null; state.sync = null; }
  const members = c ? new Set(c.nodes) : null;
  applySelection(members ? (n) => members.has(n) : null, fly);
}

/** Select one group of the ALU, as a set. */
function selectAlu(id, { fly = false } = {}) {
  const c = id === null ? null : aluList().find((x) => x.id === id);
  state.alu = c ? c.id : null;
  if (c) { state.block = null; state.stem = null; state.cluster = null; state.stage = null; state.control = null; state.pin = null; state.chain = null; state.clock = null; state.intr = null; state.branch = null; state.decimal = null; state.reg = null; state.incr = null; state.flag = null; state.alat = null; state.dbus = null; state.irp = null; state.sbus = null; state.sdp = null; state.rdy = null; state.pcr = null; state.pipe = null; state.sync = null; }
  const members = c ? new Set(c.nodes) : null;
  applySelection(members ? (n) => members.has(n) : null, fly);
}

/** Select one group of the address latches, as a set. */
function selectAlat(id, { fly = false } = {}) {
  const c = id === null ? null : alatList().find((x) => x.id === id);
  state.alat = c ? c.id : null;
  if (c) { state.block = null; state.stem = null; state.cluster = null; state.stage = null; state.control = null; state.pin = null; state.chain = null; state.clock = null; state.intr = null; state.branch = null; state.decimal = null; state.reg = null; state.incr = null; state.flag = null; state.alu = null; state.dbus = null; state.irp = null; state.sbus = null; state.sdp = null; state.rdy = null; state.pcr = null; state.pipe = null; state.sync = null; }
  const members = c ? new Set(c.nodes) : null;
  applySelection(members ? (n) => members.has(n) : null, fly);
}

/** Select one group of the flag logic, as a set. */
function selectFlag(id, { fly = false } = {}) {
  const c = id === null ? null : flagsList().find((x) => x.id === id);
  state.flag = c ? c.id : null;
  if (c) { state.block = null; state.stem = null; state.cluster = null; state.stage = null; state.control = null; state.pin = null; state.chain = null; state.clock = null; state.intr = null; state.branch = null; state.decimal = null; state.reg = null; state.incr = null; state.alat = null; state.alu = null; state.dbus = null; state.irp = null; state.sbus = null; state.sdp = null; state.rdy = null; state.pcr = null; state.pipe = null; state.sync = null; }
  const members = c ? new Set(c.nodes) : null;
  applySelection(members ? (n) => members.has(n) : null, fly);
}
...
function selectIncr(id, { fly = false } = {}) {
  ...
  if (c) { state.block = null; state.stem = null; state.cluster = null; state.stage = null; state.control = null; state.pin = null; state.chain = null; state.clock = null; state.intr = null; state.branch = null; state.decimal = null; state.reg = null; state.flag = null; state.alat = null; state.alu = null; state.dbus = null; state.irp = null; state.sbus = null; state.sdp = null; state.rdy = null; state.pcr = null; state.pipe = null; state.sync = null; }
  const members = c ? new Set(c.nodes) : null;
  applySelection(members ? (n) => members.has(n) : null, fly);
}
...
function selectDecimal(id, { fly = false } = {}) {
  ...
  if (c) { state.block = null; state.stem = null; state.cluster = null; state.stage = null; state.control = null; state.pin = null; state.chain = null; state.clock = null; state.intr = null; state.branch = null; state.reg = null; state.incr = null; state.flag = null; state.alat = null; state.alu = null; state.dbus = null; state.irp = null; state.sbus = null; state.sdp = null; state.rdy = null; state.pcr = null; state.pipe = null; state.sync = null; }
  const members = c ? new Set(c.nodes) : null;
  applySelection(members ? (n) => members.has(n) : null, fly);
}
...
function selectBranch(id, { fly = false } = {}) {
  ...
  if (c) { state.block = null; state.stem = null; state.cluster = null; state.stage = null; state.control = null; state.pin = null; state.chain = null; state.clock = null; state.intr = null; state.decimal = null; state.reg = null; state.incr = null; state.flag = null; state.alat = null; state.alu = null; state.dbus = null; state.irp = null; state.sbus = null; state.sdp = null; state.rdy = null; state.pcr = null; state.pipe = null; state.sync = null; }
  const members = c ? new Set(c.nodes) : null;
  applySelection(members ? (n) => members.has(n) : null, fly);
}
...
function selectIntr(id, { fly = false } = {}) {
  ...
  if (c) { state.block = null; state.stem = null; state.cluster = null; state.stage = null; state.control = null; state.pin = null; state.chain = null; state.clock = null; state.branch = null; state.decimal = null; state.reg = null; state.incr = null; state.flag = null; state.alat = null; state.alu = null; state.dbus = null; state.irp = null; state.sbus = null; state.sdp = null; state.rdy = null; state.pcr = null; state.pipe = null; state.sync = null; }
  const members = c ? new Set(c.nodes) : null;
  applySelection(members ? (n) => members.has(n) : null, fly);
}
...
function selectClock(id, { fly = false } = {}) {
  ...
  if (c) { state.block = null; state.stem = null; state.cluster = null; state.stage = null; state.control = null; state.pin = null; state.chain = null; state.intr = null; state.branch = null; state.decimal = null; state.reg = null; state.incr = null; state.flag = null; state.alat = null; state.alu = null; state.dbus = null; state.irp = null; state.sbus = null; state.sdp = null; state.rdy = null; state.pcr = null; state.pipe = null; state.sync = null; }
  const members = c ? new Set(c.nodes) : null;
  applySelection(members ? (n) => members.has(n) : null, fly);
}
...
function selectChain(id, { fly = false } = {}) {
  ...
  if (c) { state.block = null; state.stem = null; state.cluster = null; state.stage = null; state.control = null; state.pin = null; state.clock = null; state.intr = null; state.branch = null; state.decimal = null; state.reg = null; state.incr = null; state.flag = null; state.alat = null; state.alu = null; state.dbus = null; state.irp = null; state.sbus = null; state.sdp = null; state.rdy = null; state.pcr = null; state.pipe = null; state.sync = null; }
  const members = c ? new Set(c.nodes) : null;
  applySelection(members ? (n) => members.has(n) : null, fly);
}
...
function selectControl(id, { fly = false } = {}) {
  ...
  if (c) { state.block = null; state.stem = null; state.cluster = null; state.stage = null; state.pin = null; state.chain = null; state.clock = null; state.intr = null; state.branch = null; state.decimal = null; state.reg = null; state.incr = null; state.flag = null; state.alat = null; state.alu = null; state.dbus = null; state.irp = null; state.sbus = null; state.sdp = null; state.rdy = null; state.pcr = null; state.pipe = null; state.sync = null; }
  const members = c ? new Set(c.nodes) : null;
  applySelection(members ? (n) => members.has(n) : null, fly);
}
...
function selectPins(id, { fly = false } = {}) {
  ...
  if (c) { state.block = null; state.stem = null; state.cluster = null; state.stage = null; state.control = null; state.chain = null; state.clock = null; state.intr = null; state.branch = null; state.decimal = null; state.reg = null; state.incr = null; state.flag = null; state.alat = null; state.alu = null; state.dbus = null; state.irp = null; state.sbus = null; state.sdp = null; state.rdy = null; state.pcr = null; state.pipe = null; state.sync = null; }
  const members = c ? new Set(c.nodes) : null;
  applySelection(members ? (n) => members.has(n) : null, fly);
}
...
function applySelection(isMember, fly) {
  ...
  for (const [key, boxSel, cls] of [['pcr', '#tc-pcr-regions', 'tc-pg2'], ['pipe', '#tc-pipe-regions', 'tc-pp'], ['sync', '#tc-sync-regions', 'tc-sy']]) {
    for (const p of document.querySelectorAll(`${boxSel} .${cls}, ${boxSel} .${cls}-lb`)) {
      p.classList.toggle('sel', state[key] !== null && p.dataset[key] === state[key]);
    }
  }
  ...
  for (const p of document.querySelectorAll('#tc-sbus-regions .tc-yg, #tc-sbus-regions .tc-yg-lb')) {
    p.classList.toggle('sel', state.sbus !== null && p.dataset.sbus === state.sbus);
  }
  for (const p of document.querySelectorAll('#tc-irp-regions .tc-ig2, #tc-irp-regions .tc-ig2-lb')) {
    p.classList.toggle('sel', state.irp !== null && p.dataset.irp === state.irp);
  }
  ...
  for (const p of document.querySelectorAll('#tc-alat-regions .tc-ag, #tc-alat-regions .tc-ag-lb')) {
    p.classList.toggle('sel', state.alat !== null && p.dataset.alat === state.alat);
  }
  for (const p of document.querySelectorAll('#tc-flag-regions .tc-fg, #tc-flag-regions .tc-fg-lb')) {
    p.classList.toggle('sel', state.flag !== null && p.dataset.flag === state.flag);
  }
...
/** The cards for the three small kinds. */
function smallPills(c) {
  const { sch } = state;
  const L = state.levels, P = state.prevLevels;
  const high = [], moved = [];
  for (const n of c.nodes) {
  ...
  const nameOf = (n) => sch.names[n] || `#${n}`;
  const pill = (n, cls) => `<button type="button" class="tc-node ${L && L[n] ? 'up' : 'down'}${cls}" data-node="${n}">${nameOf(n)}<i>${L && L[n] ? 1 : 0}</i></button>`;
  return { high, moved, pill, nameOf, L };
}

function smallTail(c, sp) {
  return ` · ${c.nodes.length} node${c.nodes.length === 1 ? '' : 's'}: ${c.nodes.map((n) => sp.pill(n, '')).join(' ')}`
    + ` · <span class="tc-bk-moved">${sp.moved.length}</span> moved at this half-cycle · ${sp.high.length} high` + collapseButton();
}

function paintPcrCard(box) {
  const c = PCR.list().find((x) => x.id === state.pcr);
  const sp = smallPills(c);
  const { byName } = state;
  const byteOf = (stem) => { let v = 0; for (let i = 0; i < 8; i++) { const n = byName.get(`${stem}${i}`); if (n !== undefined && sp.L && sp.L[n]) v |= 1 << i; } return v; };
  let head;
  if (c.kind === 'byte') {
    const what = { pcl: 'the counter\'s low byte, eight pure bus bits held by PCLPCL and loaded from ADL', pch: 'the counter\'s high byte, the same on the high side', pclp: 'the next low byte: each bit an inverter of the incrementer\'s output', pchp: 'the next high byte' }[c.id];
    head = `<span class="tc-bk-name">the program counter, ${c.id}</span> · ${what} · <span class="mono">${c.id}</span> holds <b class="mono">$${hex2(byteOf(c.id))}</b> (PC reads <span class="mono">$${state.regs ? hex4(state.regs.pc) : '????'}</span>)`
      + ` · its switches sit under ${c.lines.map((n) => sp.pill(n, ' tc-hi')).join(' ')}`;
  } else {
    const open = sp.L && sp.L[c.node] > 0;
    const what = { PCLPCL: 'write the next low byte back into the counter: the hold', ADLPCL: 'load the counter\'s low byte from the address bus: a jump landing', PCLADL: 'drive the next low byte onto the address bus: the next fetch', PCHPCH: 'write the next high byte back: the hold', ADHPCH: 'load the high byte from the bus', PCHADH: 'drive the next high byte onto the bus' }[c.id];
    head = `<span class="tc-bk-name">program counter line ${c.id}</span> · ${what} · the line reads ${open ? 1 : 0}, so its ${c.switches} switches are ${open ? '<b class="tc-bk-active">open</b>' : 'shut'} now`
      + ` · made from ${c.reads.map((n) => sp.pill(n, ' tc-hi')).join(' ')}`;
  }
  const html = head + smallTail(c, sp);
  if (box.dataset.html !== html) { box.innerHTML = html; box.dataset.html = html; }
}

function paintPipeCard(box) {
  const c = PIPES.list().find((x) => x.id === state.pipe);
  const sp = smallPills(c);
  const head = c.id === 'named'
    ? `<span class="tc-bk-name">the pipeline latches, named</span> · every node the die calls pipe-something is a latch under cclk, and these fifteen carry meanings the other containers tell: the timing chain\'s, the vectors\', the write and reset latches`
    : `<span class="tc-bk-name">the pipeUNK file</span> · the thirty-seven latches the reverse engineers could not yet name, all under cclk: the file where the decoder\'s phase-1 answers are re-timed onto phase 2`;
  const html = head + ` · reads ${c.reads.length} gates, feeds ${c.feeds.length} nodes` + smallTail(c, sp);
  if (box.dataset.html !== html) { box.innerHTML = html; box.dataset.html = html; }
}

function paintSyncCard(box) {
  const c = SYNCG.list().find((x) => x.id === state.sync);
  const sp = smallPills(c);
  const { byName } = state;
  const head = `<span class="tc-bk-name">the SYNC generator</span> · the pin that says this cycle is an opcode fetch: SYNC reads <b class="mono">${state.regs ? state.regs.sync : '?'}</b> now`
    + ` · four nodes, a push-pull pad driven by one inverter, and the whole generator reads exactly one wire: ${c.reads.map((n) => sp.pill(n, ' tc-hi')).join(' ')}, the hidden T1 the T-state readout never names`;
  const html = head + smallTail(c, sp);
  if (box.dataset.html !== html) { box.innerHTML = html; box.dataset.html = html; }
}
...
/**
 * The card for a store-pipeline group: the latch's level (the readout's SD1
 * and SD2, active high), the detect with which terms set it, and what each
 * feeds, which is the holds a store imposes.
 */
function paintSdpCard(box) {
  const c = sdpList().find((x) => x.id === state.sdp);
  const { sch } = state;
  const L = state.levels, P = state.prevLevels;
  const high = [], moved = [];
  for (const n of c.nodes) {
  ...
  const nameOf = (n) => sch.names[n] || `#${n}`;
  const pill = (n, cls) => `<button type="button" class="tc-node ${L && L[n] ? 'up' : 'down'}${cls}" data-node="${n}">${nameOf(n)}<i>${L && L[n] ? 1 : 0}</i></button>`;
  const on = L && L[c.node] > 0;
  let head;
  if (c.id === 'detect') {
    head = `<span class="tc-bk-name">the store detect</span> · a NOR over the five data-cycle terms (one per addressing mode) and the RMW classes: the memory data cycle of a store or a read-modify-write, read off the wires; it also makes <span class="mono">op-rmw</span>`
      + ` · reads ${c.reads.map((n) => pill(n, ' tc-hi')).join(' ')}`;
  } else {
    const which = c.id.toUpperCase();
    head = `<span class="tc-bk-name">${which}</span> · ${c.id === 'sd1' ? 'the first store-data latch: the detect under cclk, an AOI that holds when the chip is not ready, and the node the timing readout prints as SD1' : 'the second: SD1 delayed one cycle when the chip is ready, the readout\'s SD2'}`
      + ` · ${which} reads <b class="mono">${on ? 1 : 0}</b> now (active high, the bracket in the fixed-width trace)`
      + ` · feeds ${c.feeds.map((n) => pill(n, ' tc-hi')).join(' ')}${c.id === 'sd1' ? ', the write control, the address hold and the RMW shift gating' : ', the write control and the C flag\'s shift path'}`;
  }
  const html = head
    + ` · ${c.nodes.length} node${c.nodes.length === 1 ? '' : 's'}: ${c.nodes.map((n) => pill(n, '')).join(' ')}`
    + ` · <span class="tc-bk-moved">${moved.length}</span> moved at this half-cycle · ${high.length} high` + collapseButton();
...
/**
 * The card for a special-bus group: the bus with its byte and its thirteen
 * lines by direction with their levels, the adjusted copies with the byte A
 * would load, the line sets, and SBADH with what makes it.
 */
function paintSbusCard(box) {
  const c = sbusList().find((x) => x.id === state.sbus);
  const { sch, byName } = state;
  const L = state.levels, P = state.prevLevels;
  const high = [], moved = [];
  for (const n of c.nodes) {
  ...
  const nameOf = (n) => sch.names[n] || `#${n}`;
  const short = (n) => nameOf(n).replace(/^dpc\d+_/, '');
  const pill = (n, cls, text) => `<button type="button" class="tc-node ${L && L[n] ? 'up' : 'down'}${cls}" data-node="${n}">${text || nameOf(n)}<i>${L && L[n] ? 1 : 0}</i></button>`;
  const byteOf = (stem) => { let v = 0; for (let i = 0; i < 8; i++) { const n = byName.get(`${stem}${i}`); if (n !== undefined && L && L[n]) v |= 1 << i; } return v; };
  const st = state.sbusStats;
  const lineList = (ls) => ls.map((l) => pill(l.node, ' tc-hi', `${short(l.node)} ${l.dir === 'onto' ? '→SB' : l.dir === 'off' ? 'SB→' : '↔'}`)).join(' ');
  let head;
  if (c.id === 'sb') {
    head = `<span class="tc-bk-name">the special bus</span> · <span class="mono">sb</span> carries <b class="mono">$${hex2(byteOf('sb'))}</b> now · eight pure bus bits, no gate drives them, precharged to vcc under cclk by ${st.precharge} switches`
      + ` · ${c.lines.length} lines, each with its direction read off the far side of its switches: ${lineList(c.lines)}`;
  } else if (c.id === 'dasb') {
    let adj = byteOf('sb');
    for (const i of [1, 2, 3, 5, 6, 7]) { const n = byName.get(`dasb${i}`); if (n !== undefined && L) adj = L[n] ? adj | (1 << i) : adj & ~(1 << i); }
    head = `<span class="tc-bk-name">the adjusted special bus</span> · dasb1..3 and dasb5..7, each a gate combining its bit (through an inverter) with the decimal adjust; SBAC opens these onto A with sb0 and sb4 direct, so A would load <b class="mono">$${hex2(adj)}</b> now`
      + ` · reads ${c.reads.map((n) => pill(n, ' tc-hi')).join(' ')}`;
  } else if (c.kind === 'lines') {
    const what = { onto: 'the lines whose far side is driven by a static gate: they bring a value onto the bus', off: 'the lines whose far side is a latch or a precharged bus: they take the value off', link: 'the line whose far side is another bus: it joins the special bus to the data bus' }[c.id];
    const hi = c.lines.filter((l) => L && L[l.node] > 0);
    head = `<span class="tc-bk-name">special bus, ${c.label}</span> · ${what}: ${lineList(c.lines)} · ${hi.length} open at this half-cycle${c.id === 'off' ? ' (SBADH is drawn with its own cone)' : ''}`;
  } else {
    const open = L && L[c.node] > 0;
    head = `<span class="tc-bk-name">SBADH</span> · the special bus onto the high address byte, the page fix of a taken branch and the indexed modes · the line reads ${open ? 1 : 0}, so its ${c.switches} switches are ${open ? '<b class="tc-bk-active">open</b>' : 'shut'} now · made from ${c.reads.map((n) => pill(n, ' tc-hi')).join(' ')}`;
  }
  const html = head
    + ` · ${c.nodes.length} node${c.nodes.length === 1 ? '' : 's'}: ${c.nodes.map((n) => pill(n, '')).join(' ')}`
    + ` · <span class="tc-bk-moved">${moved.length}</span> moved at this half-cycle · ${high.length} high` + collapseButton();
...
/**
 * The card for an instruction-register group: the predecode and the register
 * with their bytes off the chip, the load path with fetch's level, the
 * predecoder with which terms are high, and the named extras.
 */
function paintIrpCard(box) {
  const c = irpList().find((x) => x.id === state.irp);
  const { sch, byName } = state;
  const L = state.levels, P = state.prevLevels;
  const high = [], moved = [];
  for (const n of c.nodes) {
  ...
  const pill = (n, cls) => `<button type="button" class="tc-node ${L && L[n] ? 'up' : 'down'}${cls}" data-node="${n}">${nameOf(n)}<i>${L && L[n] ? 1 : 0}</i></button>`;
  const byteOf = (stem) => { let v = 0; for (let i = 0; i < 8; i++) { const n = byName.get(`${stem}${i}`); if (n !== undefined && L && L[n]) v |= 1 << i; } return v; };
  const st = state.irpStats;
  let head;
  if (c.id === 'pd') {
    head = `<span class="tc-bk-name">the predecode latch</span> · <span class="mono">pd</span> holds <b class="mono">$${hex2(byteOf('pd'))}</b> off the pads, and <span class="mono">pd.clearIR</span> is that byte with clearIR forcing $00, BRK, for an interrupt`
      + ` · three nodes a bit · reads ${c.reads.map((n) => pill(n, ' tc-hi')).join(' ')}`;
  } else if (c.id === 'load') {
    const f = byName.get('fetch');
    head = `<span class="tc-bk-name">the IR load path</span> · from the predecode byte into the register under <span class="mono">fetch</span>, which reads ${L && L[f] ? 1 : 0} now`
      + ` · three nodes a bit: the inverter of pd.clearIR, the node under fetch, and the register bit\'s input node that notir recirculates into under cclk`;
  } else if (c.id === 'ir') {
    head = `<span class="tc-bk-name">the instruction register</span> · IR reads <b class="mono">$${state.regs ? hex2(state.regs.ir) : '??'}</b> off the chip (the bits <span class="mono">$${hex2(byteOf('ir'))}</span>)`
      + ` · ir and notir, feeding all ${st.terms} product terms of the decode PLA directly and, through irline3, ${st.line3} of them again`;
  } else if (c.id === 'pre') {
    const hi = c.roots.filter((n) => L && L[n] > 0);
    head = `<span class="tc-bk-name">the predecoder</span> · reads the predecode byte for what the timing chain wants early: a two-cycle instruction (to #TWOCYCLE, the T0 cell) and a one-byte one (ONEBYTE, to the PC increment enable)`
      + ` · ${c.roots.map((n) => pill(n, ' tc-hi')).join(' ')} · ${hi.length} high now · feeds ${c.feeds.map((n) => pill(n, '')).join(' ')}`;
  } else if (c.id === 'irline3') {
    head = `<span class="tc-bk-name">irline3</span> · a derived IR line, ir0 and ir1 both low, that the decoder reads on ${st.line3} terms instead of the two bits · reads ${c.reads.map((n) => pill(n, ' tc-hi')).join(' ')}`;
  } else if (c.id === 'clear') {
    head = `<span class="tc-bk-name">clearIR</span> · a NAND of fetch and D1x1: on an interrupt the predecode byte is forced to $00, which is BRK, and the interrupt runs the BRK sequence · reads ${c.reads.map((n) => pill(n, ' tc-hi')).join(' ')}`;
  } else {
    head = `<span class="tc-bk-name">fetch</span> · the load line: opens the predecode byte into the register, made from ${c.reads.map((n) => pill(n, ' tc-hi')).join(' ')}`;
  }
  const html = head
    + ` · ${c.nodes.length} node${c.nodes.length === 1 ? '' : 's'}: ${c.nodes.map((n) => pill(n, '')).join(' ')}`
    + ` · <span class="tc-bk-moved">${moved.length}</span> moved at this half-cycle · ${high.length} high` + collapseButton();
...
/**
 * The card for an ALU group: a slice with the bit it computes read off the
 * chip, the input latches with their bytes, the ends with their levels, a
 * line group with which lines are high; nodes, reads and what moved.
 */
function paintAluCard(box) {
  const c = aluList().find((x) => x.id === state.alu);
  const { sch, byName } = state;
  const L = state.levels, P = state.prevLevels;
  const high = [], moved = [];
  for (const n of c.nodes) {
  ...
  let head;
  if (c.kind === 'bit') {
    head = `<span class="tc-bk-name">ALU bit ${c.bit}</span> · one slice of the adder: the hold bit, its inverse, the result select, the sum, the XOR, the OR and AND terms, the carry into this bit and its propagate term, by the die\'s own names`
      + ` · <span class="mono">alu${c.bit}</span> reads ${L && L[byName.get(`alu${c.bit}`)] ? 1 : 0} (ALU holds <span class="mono">$${hex2(byteOf('alu'))}</span>)`
      + ` · reads ${c.reads.map((n) => pill(n, ' tc-hi')).join(' ')}`;
  } else if (c.kind === 'shared') {
    head = `<span class="tc-bk-name">ALU, the generate terms</span> · #A.B1..7, each read by its own bit\'s XOR and the next bit\'s carry at the same depth, so shared between slices`
      + ` · reads ${c.reads.map((n) => pill(n, ' tc-hi')).join(' ')}`;
  } else if (c.kind === 'input') {
    const stem = c.id === 'a' ? 'alua' : 'alub';
    head = `<span class="tc-bk-name">ALU input ${c.id.toUpperCase()}</span> · <span class="mono">${stem}</span> holds <span class="mono">$${hex2(byteOf(stem))}</span>`
      + (c.id === 'b' ? ', sixteen nodes because an inverter a bit serves nDBADD, the inverted data bus that makes SBC' : ', its eight bits alone')
      + ` · loaded under ${c.lines.map((n) => pill(n, ' tc-hi')).join(' ')} · reads ${c.reads.map((n) => pill(n, ' tc-hi')).join(' ')}`;
  } else if (c.kind === 'end') {
    const what = { cin: 'the carry in, and the logic that chooses it', cout: 'the carry out of bit 7 and its copies', vout: 'the overflow out' }[c.id];
    head = `<span class="tc-bk-name">ALU ${ALU_LABEL[c.id]}</span> · ${what}: ${c.roots.map((n) => pill(n, ' tc-hi')).join(' ')}`
      + ` · reads ${c.reads.map((n) => pill(n, ' tc-hi')).join(' ')} · feeds ${c.feeds.map((n) => pill(n, ' tc-hi')).join(' ')}`;
  } else {
    const what = { in: 'the five ways into the adder', fn: 'the five functions', out: 'the three ways out, bit 7 apart because it is the shifter' }[c.id];
    const hi = c.lines.filter((n) => L && L[n] > 0);
    head = `<span class="tc-bk-name">ALU ${ALU_LABEL[c.id]}</span> · ${what}, each with the cone that makes it: ${c.lines.map((n) => pill(n, ' tc-hi')).join(' ')}`
      + ` · ${hi.length} high at this half-cycle${hi.length ? ': ' + hi.map(nameOf).join(', ') : ''}`
      + ` · made from ${c.reads.map((n) => pill(n, '')).join(' ')}`;
  }
  const html = head
    + ` · ${c.nodes.length} node${c.nodes.length === 1 ? '' : 's'}: ${c.nodes.map((n) => pill(n, '')).join(' ')}`
    + ` · <span class="tc-bk-moved">${moved.length}</span> moved at this half-cycle · ${high.length} high` + collapseButton();
...
/**
 * The card for an address-latch group: a half with the byte it holds on the
 * pins, the bus it reads and the pads it feeds; a load line with whether its
 * switches are open and what it is made from; a constant group with what it
 * forces and what it is made from.
 */
function paintAlatCard(box) {
  const c = alatList().find((x) => x.id === state.alat);
  const { sch } = state;
  const L = state.levels, P = state.prevLevels;
  const high = [], moved = [];
  for (const n of c.nodes) {
  ...
  let head;
  if (c.kind === 'half') {
    const ab = state.regs ? state.regs.ab : 0;
    const byte = c.id === 'abl' ? ab & 0xff : ab >> 8;
    head = `<span class="tc-bk-name">address latch ${c.label}</span> · holds <b class="mono">$${hex2(byte)}</b>, the ${c.id === 'abl' ? 'low' : 'high'} byte of the address on the pins (AB <span class="mono">$${hex4(ab)}</span>)`
      + ` · ${c.nodes.length} nodes, seven a bit: the bus bit\'s inverter, a node under cp1, the latch input under the load line, its inverse, the output latch under cclk, and the two static nodes of the pad\'s push-pull driver`
      + ` · reads ${c.reads.map((n) => pill(n, ' tc-hi')).join(' ')} · feeds ${c.feeds.map((n) => pill(n, ' tc-hi')).join(' ')}`;
  } else if (c.kind === 'line') {
    const open = L && L[c.node] > 0;
    head = `<span class="tc-bk-name">address latch load, ${c.id}</span> · loads ${c.half.toUpperCase()} from the ${c.half === 'abl' ? 'low' : 'high'} address bus`
      + ` · the line reads ${open ? 1 : 0}, so its ${c.onHalf} switches on the latch are ${open ? '<b class="tc-bk-active">open</b>' : 'shut'} now (${c.switches} in all)`
      + ` · made from ${c.reads.map((n) => pill(n, ' tc-hi')).join(' ')}`;
  } else {
    head = `<span class="tc-bk-name">address constants, ${c.id === 'low' ? 'ADL' : 'ADH'}</span> · ${c.id === 'low' ? 'the three low address bits forced for the vectors at $FFFA..$FFFF, inverters of the vector bits' : 'the high byte forced to $00 for the zero page and $01 for the stack'}`
      + ` · ${c.roots.map((n) => pill(n, ' tc-hi')).join(' ')} · made from ${c.reads.map((n) => pill(n, ' tc-hi')).join(' ')}`;
  }
  const html = head
    + ` · ${c.nodes.length} node${c.nodes.length === 1 ? '' : 's'}: ${c.nodes.map((n) => pill(n, '')).join(' ')}`
    + ` · <span class="tc-bk-moved">${moved.length}</span> moved at this half-cycle · ${high.length} high` + collapseButton();
...
/**
 * The card for a flag group: the flag with its bit read off P, what sets,
 * clears, loads and holds it as pills with levels; the shared enables; Pout.
 */
function paintFlagCard(box) {
  const c = flagsList().find((x) => x.id === state.flag);
  const { sch } = state;
  const L = state.levels, P = state.prevLevels;
  const high = [], moved = [];
  for (const n of c.nodes) {
  ...
  let head;
  if (c.kind === 'flag') {
    const bitv = state.regs ? (state.regs.p >> c.bit) & 1 : undefined;
    head = `<span class="tc-bk-name">flag ${c.id}</span> · <span class="mono">p${c.bit}</span> reads ${bitv === undefined ? '?' : bitv} in P (<span class="mono">$${state.regs ? hex2(state.regs.p) : '??'}</span>, <span class="mono">${state.regs ? state.regs.flags : ''}</span>) off the chip`
      + ` · ${FLAGS_WHAT[c.id] || 'a flag'}`
      + ` · reads ${c.reads.map((n) => pill(n, ' tc-hi')).join(' ')}`;
  } else if (c.kind === 'shared') {
    head = `<span class="tc-bk-name">flag logic shared by ${c.of.join(', ')}</span> · in the cone of more than one flag: the polarity of the set/clear pairs read off ir5, and the PLP/RTI load`
      + ` · reads ${c.reads.map((n) => pill(n, ' tc-hi')).join(' ')}`;
  } else {
    head = `<span class="tc-bk-name">P onto the data bus</span> · the Pout bits, each the inverse of a flag\'s pipeline copy, opened onto idb under H1x1 for PHP and the interrupt push`
      + ` · reads ${c.reads.map((n) => pill(n, ' tc-hi')).join(' ')}`;
  }
  const html = head
    + ` · ${c.nodes.length} node${c.nodes.length === 1 ? '' : 's'}: ${c.nodes.map((n) => pill(n, '')).join(' ')}`
...
function draw() {
  ...
  // The ALU's beads go under the decimal correction's outline, which outranks
  ...
  // The last three are clicked before the control clusters and above the
  // incrementer, so they draw above it: z-order agrees with click priority.
  SYNCG.draw(el('g', { class: 'tc-sync-regions', id: 'tc-sync-regions' }, cam));
  PIPES.draw(el('g', { class: 'tc-pipe-regions', id: 'tc-pipe-regions' }, cam));
  PCR.draw(el('g', { class: 'tc-pcr-regions', id: 'tc-pcr-regions' }, cam));
  drawAlatRegions(el('g', { class: 'tc-alat-regions', id: 'tc-alat-regions' }, cam));
  drawFlagsRegions(el('g', { class: 'tc-flag-regions', id: 'tc-flag-regions' }, cam));
  ...
  else if (state.pcr !== null) selectPcr(state.pcr);
  else if (state.pipe !== null) selectPipe(state.pipe);
  else if (state.sync !== null) selectSync(state.sync);
  applyCollapse();
...
function flagsCaption() {
  const st = state.flagStats;
  if (!st) return '';
  return `The ringed beads are the status register and its flag logic, each bit with the cone that sets, clears, loads and holds it `
    + `(${st.groups.filter(([id]) => !['shared', 'out'].includes(id)).map(([id, n]) => `${id} ${n}`).join(', ')}), `
    + `${st.groups.find(([id]) => id === 'shared')[1]} nodes shared by more than one flag, and the ${st.groups.find(([id]) => id === 'out')[1]} Pout bits that push P onto the data bus; B is an inverter of the timing chain\'s D1x1, not a stored bit. `
    + alatCaption();
}

function alatCaption() {
  const st = state.alatStats;
  if (!st) return '';
  return `The blue beads are the address latches, seven nodes a bit from the bus\'s inverter to the pad\'s driver (${st.halves.map(([id, n]) => `${id.toUpperCase()} ${n}`).join(', ')}), `
    + `their load lines with what makes each (${st.lines.map(([id, n, k]) => `${id} ${n}, holding ${k} switches`).join('; ')}), `
    + `and the constant generators that force address bits (${st.consts.map(([id, n, k]) => `${id} ${n} from ${k} named`).join(', ')}). `
    + aluCaption();
}

function aluCaption() {
  const st = state.aluStats;
  if (!st) return '';
...
function irpCaption() {
  ...
  return `The gold beads are the instruction register and its predecode: the predecode latch (${g('pd')}, three a bit), the load path under fetch (${g('load')}), `
    + `the register (${g('ir')}, feeding all ${st.terms} decode terms and ${st.line3} of them again through irline3), the predecoder (${g('pre')}), and irline3, clearIR and fetch on their own. `
    + sbusCaption();
}

function sbusCaption() {
  const st = state.sbusStats;
  if (!st) return '';
  const g = (id) => (st.groups.find(([x]) => x === id) || [0, 0])[1];
  return `The special bus is its ${g('sb')} bits, precharged by ${st.precharge} switches, with ${st.lines} lines whose direction is read off the far side of their switches `
    + `(${g('onto')} onto, ${g('off') + 1} off, ${g('link')} joining it to the data bus), the ${g('dasb')} nodes of the decimal-adjusted copy, and SBADH with its cone. `
    + sdpCaption();
}

function sdpCaption() {
  const st = state.sdpStats;
  if (!st) return '';
  const g = (id) => (st.groups.find(([x]) => x === id) || [0, 0])[1];
...
function smallCaption() {
  if (!state.pcrs || !state.pipeGroups || !state.syncs) return '';
  const pc = state.pcrs, pf = state.pipeGroups, sy = state.syncs;
  return `The program counter\'s own storage is four bytes of eight pure bits with six lines (${pc.filter((g) => g.kind === 'line').map((g) => `${g.id} ${g.nodes.length}`).join(', ')}); `
    + `the pipeline latch file is ${pf[0].nodes.length + pf[1].nodes.length} latches, every one under cclk (${pf[0].nodes.length} named, ${pf[1].nodes.length} pipeUNK); `
    + `and the SYNC generator is ${sy[0].nodes.length} nodes reading exactly one wire, the hidden T1.`;
}
...
const KINDS = ['regs', 'flags', 'alat', 'dbus', 'irp', 'sbus', 'sdp', 'rdy', 'pcr', 'pipe', 'sync', 'control', 'clock', 'intr', 'branch', 'decimal', 'alu', 'incr', 'chain', 'stem', 'stage', 'cluster', 'pins', 'block'];
...
function container(key) {
  ...
  if (kind === 'flags') {
    const c = flagsList().find((x) => x.id === id);
    return c ? { kind, id, nodes: c.nodes, label: c.kind === 'flag' ? `flag ${id}` : id === 'out' ? 'P out' : 'flags shared', css: FLAGS_COLOR[id] || STAGE_COLOR.any } : null;
  }
  if (kind === 'alat') {
    const c = alatList().find((x) => x.id === id);
    return c ? { kind, id, nodes: c.nodes, label: c.kind === 'half' ? `latch ${c.label}` : c.label, css: ALAT_COLOR[id] || STAGE_COLOR.any } : null;
  }
  if (kind === 'alu') {
  ...
  if (kind === 'irp') {
    const c = irpList().find((x) => x.id === id);
    return c ? { kind, id, nodes: c.nodes, label: c.label, css: IRP_COLOR[id] || STAGE_COLOR.any } : null;
  }
  if (kind === 'sbus') {
    const c = sbusList().find((x) => x.id === id);
    return c ? { kind, id, nodes: c.nodes, label: c.label, css: SBUS_COLOR[id] || STAGE_COLOR.any } : null;
  }
  if (kind === 'sdp') {
  ...
  for (const [k, K, colorOf] of [['pcr', PCR, (i) => PCR_COLOR[i]], ['pipe', PIPES, (i) => PIPE_COLOR[i]], ['sync', SYNCG, () => SYNC_COLOR]]) {
    if (kind !== k) continue;
    const c = K.list().find((x) => x.id === id);
    return c ? { kind, id, nodes: c.nodes, label: c.label, css: colorOf(id) || STAGE_COLOR.any } : null;
  }
  return null;
}
...
function selectedKey() {
  ...
  if (state.pcr !== null) return `pcr:${state.pcr}`;
  if (state.pipe !== null) return `pipe:${state.pipe}`;
  if (state.sync !== null) return `sync:${state.sync}`;
...
function selectKey(key) {
  ...
  else if (c.kind === 'pcr') selectPcr(c.id);
  else if (c.kind === 'pipe') selectPipe(c.id);
  else if (c.kind === 'sync') selectSync(c.id);
}
...
function setSmallRegions(key, btn, boxId, on) {
  state[`${key}Regions`] = on;
  $(btn).setAttribute('aria-pressed', on ? 'true' : 'false');
  $(boxId)?.classList.toggle('off', !on);
}
...
function setSdpRegions(on) {
  ...
  $('tc-sdp-regions')?.classList.toggle('off', !on);
}

function setSbusRegions(on) {
  state.sbusRegions = on;
  $('tc-sbus-btn').setAttribute('aria-pressed', on ? 'true' : 'false');
  $('tc-sbus-regions')?.classList.toggle('off', !on);
}

function setIrpRegions(on) {
  state.irpRegions = on;
  $('tc-irp-btn').setAttribute('aria-pressed', on ? 'true' : 'false');
  $('tc-irp-regions')?.classList.toggle('off', !on);
}
...
function setAluRegions(on) {
  ...
  $('tc-alu-regions')?.classList.toggle('off', !on);
}

function setAlatRegions(on) {
  state.alatRegions = on;
  $('tc-alat-btn').setAttribute('aria-pressed', on ? 'true' : 'false');
  $('tc-alat-regions')?.classList.toggle('off', !on);
}

function setFlagRegions(on) {
  state.flagRegions = on;
  $('tc-flags-btn').setAttribute('aria-pressed', on ? 'true' : 'false');
  $('tc-flag-regions')?.classList.toggle('off', !on);
}
...
async function boot() {
  ...
  try {
    ...
    $('tc-flags-btn').addEventListener('click', () => setFlagRegions(!state.flagRegions));
    if (q.get('flags') === '0') setFlagRegions(false);
    $('tc-alat-btn').addEventListener('click', () => setAlatRegions(!state.alatRegions));
    ...
    $('tc-irp-btn').addEventListener('click', () => setIrpRegions(!state.irpRegions));
    if (q.get('instreg') === '0') setIrpRegions(false);
    $('tc-sbus-btn').addEventListener('click', () => setSbusRegions(!state.sbusRegions));
    ...
    for (const [key, btn, boxId, qk] of [['pcr', 'tc-pcr-btn', 'tc-pcr-regions', 'pcreg'], ['pipe', 'tc-pipe-btn', 'tc-pipe-regions', 'pipes'], ['sync', 'tc-sync-btn', 'tc-sync-regions', 'syncgen']]) {
      $(btn).addEventListener('click', () => setSmallRegions(key, btn, boxId, !state[`${key}Regions`]));
      if (q.get(qk) === '0') setSmallRegions(key, btn, boxId, false);
    }
    ...
    svg.addEventListener('click', (e) => {
      ...
      const fk = state.flagRegions ? flagsAt(pt) : null;
      ...
      const pk = state.irpRegions ? irpAt(pt) : null;
      if (pk !== null) { selectIrp(pk === state.irp ? null : pk); return; }
      const yk = state.sbusRegions ? sbusAt(pt) : null;
      ...
      const pk2 = state.pcrRegions ? PCR.at(pt) : null;
      if (pk2 !== null) { selectPcr(pk2 === state.pcr ? null : pk2); return; }
      const pp2 = state.pipeRegions ? PIPES.at(pt) : null;
      if (pp2 !== null) { selectPipe(pp2 === state.pipe ? null : pp2); return; }
      const sy2 = state.syncRegions ? SYNCG.at(pt) : null;
      if (sy2 !== null) { selectSync(sy2 === state.sync ? null : sy2); return; }
      ...
      const uc = state.aluRegions ? aluAt(pt) : null;
      if (uc !== null) { selectAlu(uc === state.alu ? null : uc); return; }
    ...
    // ?flag=C (Z, I, D, B, V, N, shared, out) selects that group of the flag logic.
    if (q.has('flag')) selectFlag(q.get('flag'), { fly: true });
    // ?alat=abl (abh, ADL/ABL, ADH/ABH, low, high) selects that group of the address latches.
    ...
    // ?ir=pd (load, ir, pre, irline3, clear, fetch) selects that group of the instruction register.
    if (q.has('ir')) selectIrp(q.get('ir'), { fly: true });
    // ?sb=sb (dasb, onto, off, link, SBADH) selects that group of the special bus.
    ...
    // ?pc=pcl (pch, pclp, pchp, PCLPCL ...) selects that group of the counter's storage.
    if (q.has('pc')) selectPcr(q.get('pc'), { fly: true });
    // ?pipes=named (unk) selects half of the pipeline latch file.
    if (q.has('pipes') && q.get('pipes') !== '0') selectPipe(q.get('pipes'), { fly: true });
    // ?syncgen=1 selects the SYNC generator.
    if (q.get('syncgen') === '1') selectSync('sync', { fly: true });
