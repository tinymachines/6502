...
| | |
|---|---|
| Simulation | Complete. 136 tests, bit-exact against the original. |
...
cargo test --workspace --profile proof   # the verification build: same
                                    ...
                                    # 136 green either way. Benches and
                                    # anything shipped stay on --release,
                                    # whose published figures were measured
                                    # there.
cargo test --workspace              # 136 tests: netlist, functional, golden,
                                    ...
                                    # datapath + replay + state + knobs + bus +
                                    # unofficial), gpu (parity;
...
cargo test -p v6502-pins --test recorded   # rung 0 on the RECORDED bus: every
                                           # golden trace replayed with its loads
                                           # withheld, the bus answering each read
                                           # from the record itself (how the NES
                                           # console's program runs on the die
                                           # with no ROM in the room), and a
                                           # WINDOW cut from each (the machine
                                           # value at its first half-cycle, the
                                           # frames, the inputs, the shadow: one
                                           # .window text) restored into and run
                                           # to its end; MUTATE=1 flips a recorded
                                           # write and the bus must refuse at that
                                           # half-cycle, in the record and the window
cargo run --release -p v6502-pins --example replay-recorded -- run.pins [half-cycles]
                                           # a console's .pins (+ .stim beside it)
                                           # through rung 0: agreement, or the
                                           # first half-cycle that differs and the
                                           # instruction executing; a record with
                                           # no stimulus is refused; MUTATE=1;
                                           # RDY_RISE_SHIFT=n is an experiment knob
cargo run --release -p v6502-pins --example replay-recorded -- run.pins --window A B out.window
                                           # cut a window A..=B (ending on a phi2)
                                           # the service (WINDOW <name>), the wasm
                                           # machine (Machine.fromWindow) and the
                                           # Halfshot page (?window=name, from
                                           # web/windows/) stand inside; the two
                                           # shipped are the family's test cartridge
                                           # polling its pad
...
cargo test --release -p v6502-compiled     # rung 2: the pin golden through lane 0,
                                           # lane independence, and a mid-run machine
                                           # value crossed with rung 0 both ways, held
                                           # at the pins, the watched gates and memory
                                           # (never at nodes); MUTATE=1
cargo run --release -p v6502-compiled --example bench   # per machine and per 64
...
cargo test -p v6502-micro                  # rung 3: the recorded table proven on
                                           ...
                                           # MUTATE_SEAM=1 must go red); and a
                                           # branch taken on its page does not
                                           # poll in the cycle it is taken, an
                                           # interrupt there waiting one
                                           # instruction (tests/branch_interrupt.rs,
                                           # every edge around three branches on
...
cargo run --release -p v6502-micro --example brk-nmi-probe -- [op] [nmi|irq]
                                           ...
                                           # opcode, where the vector read lands;
                                           # PULSE=n for a short low, NOPS=n for
                                           # the sled. The sample-point rules
                                           # were measured here.
cargo run --release -p v6502-micro --example probe3 -- <hex> [half-cycles]
                                           # rung 3 alone through a few bytes:
                                           # registers, stack page, zero page
cargo run --release -p v6502-sim --example reset-probe  # rung 0 through the
                                           ...
                                           # measurement rung 3's reset is from
cargo run --release -p v6502-micro --example bench      # rung 3 beside rung 0
cargo run --release -p v6502-micro --example diverge -- rom.nes [half-cycles]
                                           # rung 0 and rung 3 in lockstep on an
                                           # iNES image until pins or registers
                                           # disagree: how a program that fails
                                           # on rung 3 alone is located, not
                                           # reasoned about (two blargg finds)
cargo run --release -p v6502-sim --example alu-probe [hex] [half-cycles]
                                           # rung 0's ALU register, input latches,
...
cargo test --release -p v6502-gpu          # GPU lane k == CPU lane k, every node
                                           ...
                                           # sparse pages; a spent pool must
                                           # refuse; MUTATE=1
...
| `v6502-compiled` | Rung 2: `build.rs` derives the schematic and emits the kernel as Rust (gates as sum-of-products, switches unrolled), 64 machines per word on the `halfphi::slice` encoding. No engine crate at run time. Held to the pin golden, not to rung 0's nodes. The generated file is NC-SA-derived and never committed. Also emits the kernel as WGSL (`KERNEL_WGSL`). Carries the machine value across the rung boundary (`State::extract_lane`/`inject_all`, broadcast: the clock is one instruction for all lanes), held at pins, gates and memory, never at nodes. |
...
| `v6502-micro` | Rung 3: no nodes. `build.rs` runs rung 0 over 256 opcodes x 10 contexts and records the control-vector table (spans keyed by opcode and six authored selector bits, minimal per-op masks by exhaustive search, seam write-back words, the reset seed classified against two vectors); the datapath, flags, P-to-stack timing and the input pins (interrupts as the BRK span hijacked, RDY, SO, the warm reset's measured freewheel) are authored and labelled. Held to the whole pin golden, stimulus traces included. The generated table is NC-SA-derived and never committed. |
...
- **A search over recordings can only see what was recorded, and a setup
  ...
  produce.
- **Fixing that opened a worse hole in the same wall within the hour,
  and every gate here stayed green through it.** The new contexts gave
  BMI taken cases on its page and across it but none taken, forward, on
  its page, so no pair of recordings needed the TAKEN bit, the search
  dropped it, and `LDA #$80 / BMI` played the not-taken span. 135 tests,
  the 289-trace pin golden and blargg's branch-timing ROMs were all
  green; a game found it. **When a derived table drops a bit, assert the
  invariant, not the search's conclusion**: taken and not taken are
  different lengths, so no recording can make that bit redundant, and
  `tests/branch_page.rs` now says so in one line. The same anomaly had
  been printed and read aloud an hour earlier and reasoned away as
  single-valued, which it was: single-valued over the recordings is not
  the claim that matters.
- **A test written from the same list as the code tests the same three
  cases and misses the same fourth.** The branch lockstep covered
  backward-across, forward-across and backward-on-page, exactly the
  cases the recording contexts covered, and left out the commonest
  branch on the chip. Enumerate the space, not the interesting part of
  it.
