...
const CONTEXTS: &[(&str, u16, &[u8], &[u8])] = &[
    ...
    // A taken branch with a NEGATIVE offset that crosses a page backward
    // (the op just past a page start, the offset reaching into the page
    // before): three flag sets so every branch takes it once. blargg's
    // NMI tests loop backward across $E900 (tinymachines/nes, N5).
    ("bnegcross", 0x0300, &[0xa9, 0x01, 0xa2, 0x02, 0xa0, 0x03, 0x18], &[0xf0, 0x02, 0x00]),
    ("bnegcross_cz", 0x0300, &[0xa9, 0x00, 0xa2, 0x02, 0xa0, 0x03, 0x38], &[0xf0, 0x02, 0x00]),
    ("bnegcross_nv", 0x0300, &[0xa9, 0xc0, 0x85, 0x10, 0x24, 0x10, 0xa2, 0x02], &[0xf0, 0x02, 0x00]),
];
