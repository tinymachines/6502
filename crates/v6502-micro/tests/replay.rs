...
//!
//! The stimulus traces (scripted res/irq/nmi/rdy runs, a `.stim` beside the
//! `.pins`) are skipped by name: inputs are not yet authored, and that gap
//! is recorded here rather than papered over. Everything else must match
//! rung 0's recording exactly, except the opcodes in `EXPECTED_FAILURES`,
//! which are listed by number per the ladder's rule for undocumented
//! behaviour the datapath cannot express; an unexpected pass there fails
...
fn the_pin_golden_replays_through_rung3() {
    ...
    for path in &names {
        if path.with_extension("stim").exists() {
            skipped += 1;
            continue;
        }
        ...
        let mut frames = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
            run(&mut cpu, trace.frames.len() as u64 - 1, &[])
        }))
        .unwrap_or_default();
    ...
    eprintln!("replay: {ran} traces through rung 3, {skipped} stimulus traces skipped (inputs not authored)");
