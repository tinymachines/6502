...
- **Verification**: the pin golden, all 289 traces, is the gate; the
...
| M3 | `v6502-compiled` | same two comparisons; lane-independence test; generated-code licence check; throughput per machine and per 64 machines recorded. **Done 2026-08-30 on the slice encoding (choice b)**: pin golden all 271 traces through lane 0, lane-independence and program-level checks, `check-compiled-nodata.py`; node agreement with rung 0 measured and reported (`examples/agree.rs`), not asserted, per the M0 correction. 220,286 machine-half-cycles/s, 6.83x rung 0 per machine, 0.11x per sweep. A power-on tie-break was needed that the kernel never met. See `docs/notes/engine.md`, "Rung 2". **And on a GPU** (`v6502-gpu`, same day): the same kernel as WGSL from the same `build.rs`, bit-exact with the CPU rung lane for lane, 3.78 M machine-half-cycles/s at 6,400 machines on an RTX 3070, about 128x rung 0; limited by the box's free GPU memory. **Sparse per-lane memory landed 2026-08-31** (shared base, copy-on-write pages, a spent pool refusing by the numbers): the ceiling moved from 6,400 machines to 512,000 on the same card, 4.95 M machine-half-cycles/s at the 128,000-machine peak, about 167x rung 0; the limit is now the serial phases inside a word. |
| M4 | `v6502-micro` | pin golden on 7 programs + 256 opcodes + stimulus files; the unresolved-line list in the note; throughput recorded. **Built 2026-08-30**: the recorder with its single-valuedness gate (six selector bits, minimal per-op masks by exhaustive search), the datapath ported from the proven model, flags and P-to-stack authored, and the pin golden minus the six stimulus traces replaying exactly, 259 of 265 with `EXPECTED_FAILURES` empty; MUTATE red on the table, the datapath and the replay. 39.0 M half-cycles/s, about 1,465x rung 0. **Closed 2026-08-31**: the input pins are authored (an interrupt is the BRK span hijacked: `#IPC` forced, B clear, the flavour's vector select through both vector cycles; RDY holds read cycles; SO sets V; the warm reset's freewheel measured first with `reset-probe` and then authored as the overlap pair replaying), and **all 274 traces replay exactly**, `UNAUTHORED_STIM` empty. **Decimal closed 2026-08-31**: three BCD chain fixtures recorded from rung 0 (every result a `STA`, every flag set a `PHP`), `decimal-probe` measured the adjust living on the SB-to-AC path (ADD holds the binary sum, A receives the adjusted one), and the authoring is a seventh selector bit (`SEL_D`), two `SED` recorder contexts, the adjust latched beside a `#DAA`/`#DSA`-asserted SUMS, and NMOS decimal flags for the ADC family. The decimal fresh context (`gen-d`) also flushed two latent data-in-the-table bugs: the overlap alucin was recorded where nothing consumes it (scenery that agreed by luck), and the RRA family's overlap carry-in is its own ROR's carry out, now computed from the sequencer's shift capture. What the build taught beyond the spec: `SRS` shifts the B input alone; DL latches written bytes while the external pin holds the last read; the ADD-path write-back lands inside the next instruction's first half-cycle (each variant records its seam word); the reset seed is vector-relative, classified against two vectors. |
| M5 | `halfwave` grows an `ENGINE rung` line-protocol word defaulting to 0, and the service exposes it | `tests/state.rs`-style resume proof for each rung that supports state; rung 3's state codec is its own, smaller, and says so. **Done 2026-08-31**: the binary moved to `v6502-halfwave` (the dependency direction demanded it; the artifact keeps its path), `ENGINE 0` is byte-for-byte the old behaviour, `ENGINE 3` speaks rung 3's own `MICRO` value, rungs 1 and 2 are refused by name with the reason, and the FastAPI half routes a step by the machine value's shape. Resume proofs stand per rung (`v6502-sim`, `v6502-hybrid`, `v6502-micro`, each `tests/state.rs`); rung 3's codec is about 90 bytes and the model documents it as its own. The 5 new service tests include a Die Runner frame on `ENGINE 3` and rung 0/3 memory-and-gates parity through the doors. |
